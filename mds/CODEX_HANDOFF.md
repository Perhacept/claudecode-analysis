...
`question` / `answer` 仍保留为第一条问答的兼容字段；渲染、编辑、复制时以 `qaItems` 为准，没有 `qaItems` 的旧记录会自动按旧字段补成一条问答。卡片顶部的“添加新一条 QA”只追加问答，不改变 `text`、`sectionId`、`scope`、`sourceId` 或 `anchor`。每条问答的答案默认展开，顶部“全部收起 / 全部展开”按钮可批量切换，单条问题行仍可单独展开或隐藏回答。如果用户要换绑定位置，应删除最后一条问答后重新选中文字添加。
